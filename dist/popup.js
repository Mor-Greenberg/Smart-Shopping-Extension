...
function normalizeBrandForDisplay(brand) {
...
function cleanProductTitle(rawTitle, brand) {
    ...
    value = value.replace(/\b(trainers|shoes|sneakers|boots|sandals|heels|sliders)\b\s*$/i, "").trim();
    return value || "not found";
}
function fallbackBrandFromTitle(rawTitle) {
...
function buildGoogleFallbackUrl(brand, product) {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        const injected = await chrome.scripting.executeScript({
            ...
            func: () => {
                ...
                const metaBrand = getMetaByProperty("product:brand") ||
                    getMetaByName("brand");
                return {
        ...
        const extractedBrand = normalizeBrandForDisplay(data.jsonLdBrand || "") ||
    ...
    searchBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            const bestLinkUrl = typeof result.bestLink === "string"
                ? result.bestLink
                : result.bestLink?.url || "";
            if (bestLinkUrl) {
