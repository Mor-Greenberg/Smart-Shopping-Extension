...
const MAX_CONCURRENCY = Number(process.env.SCRAPER_MAX_CONCURRENCY || 6);
...
const DEFAULT_SCRAPE = {
    // renderDelay is a *ceiling* on how long we wait for content to appear,
    // not a fixed delay — pollForState returns as soon as the page is usable.
    renderDelay: 3500,
    maxPrice: 9999,
    waitUntil: 'domcontentloaded',
    searchWaitTimeout: 5000,
    // Extra grace for a price to hydrate once we know we are on a product page.
    priceSettleMs: 2500,
    // Per-link ceiling while walking search results looking for the EAN.
    eanSettleMs: 1200,
    // Hard budget for one site so a slow retailer cannot stall the whole compare.
    siteBudgetMs: 15000,
    pollIntervalMs: 200,
    headless: true,
    launchArgs: [
    ...
    // Third-party tags that add seconds of load time and never carry price data.
    blockUrlPatterns: [
...
function resolveScrapeConfig(siteSettings) {
    ...
    return {
        ...
        priceSettleMs: scrape.priceSettleMs ?? DEFAULT_SCRAPE.priceSettleMs,
        eanSettleMs: scrape.eanSettleMs ?? DEFAULT_SCRAPE.eanSettleMs,
        siteBudgetMs: scrape.siteBudgetMs ?? DEFAULT_SCRAPE.siteBudgetMs,
        pollIntervalMs: scrape.pollIntervalMs ?? DEFAULT_SCRAPE.pollIntervalMs,
        ...
        blockUrlPatterns: scrape.blockUrlPatterns || DEFAULT_SCRAPE.blockUrlPatterns,
...
async function verifyEanOnPage(page, ean) {
    const found = await page.evaluate((targetEan) => {
        const html = document.documentElement.innerHTML;
        const escaped = String(targetEan).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const exact = new RegExp(`"(?:barcode|sku|gtin13|gtin|ean)"\\s*:\\s*"${escaped}"`, 'i');
        if (exact.test(html)) return true;
        const labeled = new RegExp(`(?:ברקוד|barcode|ean|gtin)[^\\d]{0,40}${escaped}(?!\\d)`, 'i');
        return labeled.test(html);
    }, ean);
    return found;
}

async function confirmProductIdentity(page, ean, sourceProductName, scrapeConfig, siteSettings) {
    let pageEan = null;
    try {
        pageEan = await extractEanOnPage(page, siteSettings.eanExtraction);
    } catch (_) { /* fall through to HTML scan */ }

    if (pageEan && pageEan === ean) {
        return { ok: true, via: 'ean' };
    }
    if (await verifyEanOnPage(page, ean)) {
        return { ok: true, via: 'ean-html' };
    }

    const nameConfig = scrapeConfig.productNameVerification;
    if (nameConfig?.enabled && sourceProductName) {
        const nameCheck = await verifyProductNameOnPage(page, sourceProductName, nameConfig);
        if (nameCheck.pass && !nameCheck.skipped) {
            return { ok: true, via: 'name', score: nameCheck.score };
        }
        return { ok: false, via: 'name', score: nameCheck.score };
    }
...
function hasUsablePrice(state) {
    return Boolean(state && state.price > 0);
}

function isProductPageReady(state) {
    return hasUsablePrice(state);
}

function isSearchPageReady(state) {
    if (!state) return false;
    if (state.onProduct) return hasUsablePrice(state);
    return Boolean(state.productLink) || state.searchResultCount > 0;
}

/**
 * Polls the page until `isReady` passes, treating `maxWaitMs` as a ceiling
 * instead of a fixed delay. Most pages are usable long before the ceiling,
 * which is where the bulk of the old per-navigation wait time went.
 */
async function pollForState(page, ean, scrapeConfig, { maxWaitMs, isReady }) {
    const deadline = Date.now() + Math.max(0, maxWaitMs);
    const interval = scrapeConfig.pollIntervalMs || 200;
    let state = null;
    let lastError = null;
...
async function waitForEanMatch(page, ean, siteSettings, scrapeConfig) {
    const deadline = Date.now() + Math.max(0, scrapeConfig.eanSettleMs);
    const interval = scrapeConfig.pollIntervalMs || 200;

    while (Date.now() < deadline) {
        try {
            const pageEan = await extractEanOnPage(page, siteSettings.eanExtraction);
            if (pageEan === ean) return true;
            if (await verifyEanOnPage(page, ean)) return true;
        } catch (_) {
            // page is mid-navigation — retry until the ceiling
        }
...
async function tryProductLinksByEan(page, ean, scrapeConfig, siteSettings, siteLabel, preCollectedLinks, sourceProductName, deadlineAt = Infinity) {
    const nav = scrapeConfig.navigation || {};
    ...
    for (const link of links.slice(0, maxAttempts)) {
        if (Date.now() >= deadlineAt) {
            log('WARN', 'Product link walk stopped: site budget exhausted', { siteLabel, ean });
            break;
        }
        ...
        try {
            ...
            const htmlMatch = await waitForEanMatch(page, ean, siteSettings, scrapeConfig);
            ...
            return pollForState(page, ean, scrapeConfig, {
                maxWaitMs: scrapeConfig.priceSettleMs,
                isReady: isProductPageReady
            });
        } catch (err) {
...
async function preparePage(page, scrapeConfig) {
    ...
    const blockUrlPatterns = scrapeConfig.blockUrlPatterns || [];
    if (blockResources.length || blockUrlPatterns.length) {
...
async function initCluster() {
    ...
    clusterInitPromise = (async () => {
        ...
        await cluster.task(async ({ page, data }) => {
            if (data.mode === 'probe') {
                return probeWithPage(page, data);
            }
...
async function scrapeWithPage(page, targetSite, ean, siteSettings, sourceProductName, scrapeConfig) {
    ...
    const siteDeadlineAt = startedAt + scrapeConfig.siteBudgetMs;
    ...
    const searchUrl = siteSettings.searchUrlPattern.replace('{{ean}}', encodeURIComponent(ean));
        ...
        let state = await pollForState(page, ean, scrapeConfig, {
            maxWaitMs: scrapeConfig.renderDelay,
            isReady: isSearchPageReady
        });
        ...
        if (state.onSearch) {
            if (state.productLink && !state.productLink.toLowerCase().startsWith('javascript:')) {
                ...
                state = await pollForState(page, ean, scrapeConfig, {
                    maxWaitMs: scrapeConfig.renderDelay,
                    isReady: isProductPageReady
                });
        ...
        // Only pay the extra settle time when the price is still missing.
        if (!state.onSearch && !hasUsablePrice(state)) {
            state = await pollForState(page, ean, scrapeConfig, {
                maxWaitMs: Math.min(
                    scrapeConfig.priceSettleMs,
                    Math.max(0, siteDeadlineAt - Date.now())
                ),
                isReady: isProductPageReady
            });
        }
        ...
        if (onProductPage) {
            const identity = await confirmProductIdentity(
                page, ean, sourceProductName, scrapeConfig, siteSettings
            );
            if (!identity.ok) {
                log('WARN', 'Product page rejected: identity mismatch', {
                    siteLabel,
                    ean,
                    productUrl,
                    via: identity.via,
                    score: identity.score ?? null
                });
                onProductPage = false;
            }
        }
...
async function probeWithPage(page, { url, siteSettings, titleSelectors }) {
    const scrapeConfig = resolveScrapeConfig(siteSettings || {});
    await preparePage(page, scrapeConfig);
    ...
    try {
        ...
        ean = parsed.searchParams.get('text')
            || parsed.searchParams.get('q')
            || parsed.searchParams.get('search')
            || 'HEALTHCHECK';
    } catch (_) { /* keep default */ }

    const isSearchUrl = /\/search/i.test(targetUrl) || /[?&](text|q|search)=/i.test(targetUrl);
    ...
    try {
        ...
        log('ERROR', 'Selector probe navigation failed', { url, error: err.message });
        return {
            ok: false,
            httpStatus,
            titleFound: false,
    ...
    const waitSelector = isSearchUrl
        ? scrapeConfig.searchWaitForSelector
        : scrapeConfig.waitForSelector;
    if (waitSelector) {
        try {
            ...
            log('WARN', 'Selector probe waitForSelector timed out', {
                url: targetUrl,
    ...
    if (state.onSearch || isSearchUrl) {
        let productUrl = state.productLink;
        if (!productUrl) {
            const links = await collectProductLinks(page, scrapeConfig);
            productUrl = links[0] || null;
        }
        if (productUrl && !productUrl.toLowerCase().startsWith('javascript:')) {
            log('INFO', 'Selector probe following product link', { from: targetUrl, productUrl });
            try {
                const productResponse = await page.goto(productUrl, {
                    waitUntil: scrapeConfig.waitUntil,
                    timeout: 45000
                });
                httpStatus = productResponse ? productResponse.status() : httpStatus;
            } catch (err) {
                log('ERROR', 'Selector probe product navigation failed', {
                    productUrl,
                    error: err.message
                });
                return {
            ...
            if (scrapeConfig.waitForSelector) {
                try {
                    await page.waitForSelector(scrapeConfig.waitForSelector, {
                        timeout: scrapeConfig.searchWaitTimeout
                    });
                } catch (_) { /* extract anyway */ }
            }
            await sleep(scrapeConfig.renderDelay);
            state = await extractFromPage(page, ean, scrapeConfig);
        }
    ...
    const title = await extractProductNameOnPage(
        page,
        titleSelectors || scrapeConfig.productNameVerification.productPageSelectors
    );
    const titleFound = Boolean(title && String(title).trim().length > 1);
    const priceFound = Number.isFinite(state.price) && state.price > 0;
    const ok = Boolean(titleFound && priceFound);

    log(ok ? 'INFO' : 'WARN', 'Selector probe complete', {
        url: targetUrl,
        httpStatus,
        titleFound,
        priceFound,
    ...
    return {
        ...
        matchedSelector: state.selector || null,
        url: state.currentUrl || targetUrl,
        onProduct: Boolean(state.onProduct)
    };
}

async function executeProbe({ url, siteSettings, titleSelectors }) {
...
app.post('/probe', async (req, res) => {
    const { url, siteSettings, titleSelectors } = req.body || {};
    if (!url) {
    ...
    try {
        const result = await executeProbe({ url, siteSettings, titleSelectors });
        res.json(result);
    } catch (err) {
        log('ERROR', 'Selector probe failed', { url, error: err.message, code: err.code });
        res.status(500).json({
            ok: false,
            titleFound: false,
            priceFound: false,
...
app.listen(PORT, () => {
    ...
    // Launch Chrome up front so the first comparison does not pay for it.
    initCluster().catch((err) => {
        log('ERROR', 'Cluster warm-up failed — will retry on first request', {
            error: err.message
        });
    });
});
